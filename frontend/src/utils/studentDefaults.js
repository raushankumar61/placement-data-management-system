...
const generatePhone = (seed) => String(1000000000 + (hashString(`${seed}-phone`) % 9000000000)).slice(0, 10);

const generateRollNo = (branch, graduationYear, seed) => {
...
const generateUsn = (branch, graduationYear, seed) => {
  const year = String(graduationYear || new Date().getFullYear()).slice(-2);
  const code = branchCode(branch);
...
export { fillStudentDefaults };
export default fillStudentDefaults;
